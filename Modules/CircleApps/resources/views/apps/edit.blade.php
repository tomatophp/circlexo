<x-tomato-admin-container label="{{trans('tomato-admin::global.crud.edit')}} {{__('App')}} #{{$model->id}}">
    <x-splade-form class="flex flex-col gap-4" action="{{route('admin.apps.update', $model->id)}}" method="post" :default="$model">
        <x-splade-select class="col-span-2" :label="__('Categories')" :placeholder="__('Categories')" name="categories" multiple choices>
            ...
            @endforeach
        </x-splade-select>        <x-splade-select class="col-span-2" :label="__('Account')" :placeholder="__('Account')" name="account_id" :remote-url="route('admin.accounts.api')" remote-root="data" option-label="name" option-value="id" choices/>
