@php
    ...
    if(!isset($options)){
        $options = \Modules\TomatoCategory\App\Models\Type::where('for', 'product-options')->where('type', 'type')->get();
    }
    if(!isset($categories)){
        $categories = \Modules\TomatoCategory\App\Models\Category::where('for', 'product-categories')->where('menu', true)->where('activated', true)->get();
    }
    if(!isset($products)){
        $products = \Modules\TomatoProducts\App\Models\Product::where('is_activated', true)->paginate(9);
    }
@endphp
<section>
