...
<section class="font-main">
    <div class="max-w-screen-xl px-4 py-12 mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @php $services = \Modules\TomatoCms\App\Models\Service::all(); @endphp
        ...
        @php
            if(!isset($projects)){
                $projects = \Modules\TomatoCms\App\Models\Portfolio::query();
        ...
        @else
            <div class="grid grid-cols-1 gap-4 mt-8 lg:gap-8 sm:grid-cols-2 lg:grid-cols-3">
                @php
                    if(!isset($projects)){
                        if($section['is_random'] ?? false){
                            $projects = \Modules\TomatoCms\App\Models\Portfolio::with('service')->where('activated', 1)->inRandomOrder()->paginate(9);
                        }
                        else {
                            $projects = \Modules\TomatoCms\App\Models\Portfolio::with('service')->where('activated', 1)->paginate(9);
                        }
                    }
                @endphp
