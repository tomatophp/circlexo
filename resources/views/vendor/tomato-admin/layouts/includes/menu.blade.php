...
<x-splade-link href="{{route('admin.profile.edit')}}" class="px-4 py-3 flex justify-start gap-4" v-show="!data.makeMenuMin">
    ...
    @endphp
    <div>
        <div class="w-14 h-14 rounded-full bg-zinc-200 bg-cover bg-center dark:bg-zinc-900" style="background-image: url('{{$grav_url}}')">
        </div>
    </div>
    <div class="flex flex-col items-center justify-center">
        <div>
            <h6 class="text-lg text-white font-bold">{{auth('web')->user()->name}}</h6>
            <p class="text-sm text-zinc-300">{{auth('web')->user()->email}}</p>
        </div>
    </div>
</x-splade-link>
<div v-show="!data.makeMenuMin" class="my-2 border-t border-zinc-700"></div>
        ...
        <div class="text-sm">
            <div>
                <Link
                    ...
                    href="{{ route('logout') }}"
                    class="
                    text-danger-400 hover:bg-zinc-500/5 focus:bg-zinc-500/5
                    flex items-center justify-center w-full py-3 font-medium transition" :class="{'gap-3 px-4': !data.makeMenuMin}">
