                    ...
                    <div class="relative z-0 mt-1 border border-zinc-200 rounded-lg cursor-pointer">
                        @foreach($availableRoles as $role)
                            <button
                                ...
                                :class="{'border-t border-zinc-200 rounded-t-none': @json(!$loop->first), 'rounded-b-none': @json(!$loop->last)}"
                                @click='form.role = @json($role->key)'
                            >
                                <div :class='{"opacity-50": form.role && form.role != @json($role->key)}'>
                                    <!-- Role Name -->
                                    <div class="flex items-center">
                                        <div class="text-sm text-zinc-600" :class='{"font-semibold": form.role == @json($role->key)}'>
                                            {{ $role->name }}
                                        </div>
                                    ...
                                    <!-- Role Description -->
                                    <div class="mt-2 text-xs text-zinc-600 text-left">
                                        {{ $role->description }}
                                    </div>
                                </div>
                            </button>
